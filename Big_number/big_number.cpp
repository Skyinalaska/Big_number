...
Big_number Big_number::Mul_by_Karacuba(Big_number& a, Big_number& b)
{
    Big_number result;
    ...
    if(a.GetLength() > b.GetLength())
    {
        ...
        X0.head = a.head; X0.tail = &a.head[n-1];
        X1.head = &a.head[n]; X1.tail = a.tail;

        if(n >= b.GetLength())
        {
            //Big_number Y0(&b.head[0], b.tail), Y1; Y1 = 0;
            Y0.head = b.head; Y0.tail = b.tail; Y1 = 0;
        }
        else
        {
            //Big_number Y0(&b.head[0], &b.head[n-1]) ,Y1(&b.head[n], b.tail);
            Y0.head = b.head; Y0.tail = &b.head[n-1];
            Y1.head = &b.head[n]; Y1.tail = b.tail;
        }
    }
    else
    {
        ...
        if(n >= a.GetLength())
        {
            ...
            X0.head = a.head; X0.tail = a.tail; X1 = 0;
        }
        else
        {
            //Big_number X0(&a.head[0], &a.head[n-1]), X1(&a.head[n], a.tail);
...
void Big_number::right_shift(int n)
{
	int i;
...
Big_number Big_number::Mod_by_Barret(Big_number& mod, Big_number& z)
{
	...
	{
		Big_number r;
		r.Resize(mod_length + 1); 
		r.tail = r.head + mod_length + 1;
		r.head[mod_length + 1] = 1;
		for (int i = 0; i < mod_length + 1; i++)
			r.head[i] = 0;
		r = r - r2;
		r = r + r1;
	}
